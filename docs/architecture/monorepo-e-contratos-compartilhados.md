...
O Monorepo consolida as aplicações de frontend (Nuxt 3) e backend (NestJS 11) juntamente com o pacote de domínio compartilhado (`@alaska/contracts`), garantindo **End-to-End Type Safety** em tempo de compilação e validação **Fail-Fast** em tempo de execução.
...
1. **Passo 1: Criação da Estrutura Raiz**:
   * Criar os diretórios `apps/web`, `apps/api`, `packages/contracts` e `packages/tsconfig`.
...
3. **Passo 3: Migração do Frontend Nuxt 3**:
...
4. **Passo 4: Migração do Backend NestJS 11**:
...
6. **Passo 6: Ajuste de CI/CD e Deploy**:
   ...
   * Atualizar `Dockerfile` do backend com `turbo prune`.
