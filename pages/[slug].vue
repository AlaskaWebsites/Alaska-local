<template>
  <div v-if="tenant" class="min-h-screen bg-gray-50 pb-28">
    ...
    <header class="relative bg-white border-b border-gray-100 shadow-sm">
      ...
      <div class="px-4 pb-4 pt-3 flex items-start gap-3">
        ...
        <div class="flex-1">
          <div class="flex items-center justify-between">
            ...
            <span 
              :class="isOpen ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'"
      ...
      <div class="px-4 py-2 bg-gray-50 border-t border-gray-100 flex items-center justify-between text-xs text-gray-600">
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mt-2">
                ...
                <span v-if="product.optionGroups?.length" class="text-[11px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full font-medium">
                  Personalizável
                </span>
              </div>
            </div>
      ...
      >
        ...
        <div class="p-4 overflow-y-auto flex-1 space-y-4">
          ...
          <div class="border-t pt-3">
            ...
            <textarea 
              ...
              class="w-full text-xs p-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
            ></textarea>
          </div>
        </div>
        ...
        <div class="p-4 border-t border-gray-100 bg-white flex items-center gap-3">
          <div class="flex items-center border border-gray-200 rounded-xl p-1 shrink-0">
            <button 
              @click="productQuantity > 1 ? productQuantity-- : null"
    ...
    >
      <div class="max-w-md mx-auto flex items-center justify-between">
        <div>
          <span class="text-xs text-gray-500 block">{{ totalItemsCount }} {{ totalItemsCount === 1 ? 'item' : 'itens' }}</span>
        ...
        <div class="p-4 overflow-y-auto flex-1 space-y-4 text-xs">
          ...
          <div class="border-t pt-3">
            ...
            <div class="grid grid-cols-2 gap-2">
              <button 
                @click="checkoutData.deliveryType = 'delivery'"
          ...
          <div class="border-t pt-3 space-y-2">
            ...
            <div v-if="checkoutData.deliveryType === 'delivery'" class="space-y-2 pt-1">
              <div class="grid grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  <input 
                    v-model="checkoutData.address.number" 
                    type="text" 
                    placeholder="123"
                    class="w-full p-2.5 rounded-xl border border-gray-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  />
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <input 
                    v-model="checkoutData.address.complement" 
                    type="text" 
                    placeholder="Apto 42"
                    class="w-full p-2.5 rounded-xl border border-gray-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  />
            ...
            <div class="pt-2">
              ...
              <div v-if="checkoutData.paymentMethod === 'Dinheiro'" class="mt-2">
                ...
                <input 
                  v-model.number="checkoutData.changeFor" 
                  type="number" 
...
const slug = (route.params.slug as string) || 'hamburgueria'

// 1. Carregamento Seguro e Compatível com SSR
const { data: tenant } = await useAsyncData(`tenant-${slug}`, async () => {
  try {
    const rawData = await import(`../data/tenants/${slug}.json`)
    return TenantSchema.parse(rawData.default)
  } catch (err) {
    console.error(`Erro ao carregar tenant [${slug}]:`, err)
    // Fallback gracioso para a demo
    const fallback = await import('../data/hamburgueria-x.json')
    return TenantSchema.parse(fallback.default)
  }
})
